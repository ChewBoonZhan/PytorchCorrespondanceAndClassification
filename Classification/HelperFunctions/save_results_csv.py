...
def save_results_csv(path,path_to_csv,true_classes, predict_classes, image_path_test):
    ...
    with open(path_to_csv, 'w', newline='') as file: 
       ...
       for i in range(len(true_classes)):
         if true_classes[i] == predict_classes[i]: #correct
           writer.writerow([image_path_test[i],true_classes[i], predict_classes[i],"True"]) # get the filename in the good seed folder, and write each row with filename and 1
